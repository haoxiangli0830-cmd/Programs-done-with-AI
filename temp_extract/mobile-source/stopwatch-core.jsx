...
function useStopwatchStore() {
  ...
  const api = React.useMemo(() => ({
    ...
    editLap(timerId, lapId, name, newDeltaMs) {
      update(timerId, (t) => {
        const lapIdx = t.laps.findIndex((l) => l.id === lapId);
        if (lapIdx === -1) return {};
        const prevAtMs = lapIdx === 0 ? 0 : t.laps[lapIdx - 1].atMs;
        const maxAtMs = lapIdx === t.laps.length - 1 ? elapsed(t, Date.now()) : t.laps[lapIdx + 1].atMs;
        
        let targetAtMs = t.laps[lapIdx].atMs;
        if (newDeltaMs !== undefined) {
           targetAtMs = Math.min(maxAtMs, prevAtMs + newDeltaMs);
        }

        const newLaps = [...t.laps];
        newLaps[lapIdx] = { ...newLaps[lapIdx], name, atMs: targetAtMs };
        return { laps: newLaps };
      });
    },
