          ...
          <div style={{ marginTop: 22 }}>
            ...
            {t.laps.length === 0 && !running && ms === 0 ? (
              ...
              <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 14, overflow: 'hidden' }}>
                {(running || ms > (t.laps.length > 0 ? t.laps[t.laps.length - 1].atMs : 0)) && (() => {
                  const currentLapDelta = ms - (t.laps.length > 0 ? t.laps[t.laps.length - 1].atMs : 0);
                  return (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: t.laps.length > 0 ? `1px solid ${COLORS.border}` : 'none', background: COLORS.surface2 }}>
                      <div style={{ fontFamily: FONT_DIGITS, fontSize: 11, color: t.color, width: 24 }}>{String(t.laps.length + 1).padStart(2, '0')}</div>
                      <div style={{ flex: 1, fontSize: 14, fontWeight: 600, color: t.color }}>Current Lap</div>
                      <div style={{ fontFamily: FONT_DIGITS, fontSize: 12, color: t.color, fontWeight: 600 }}>+{fmtHuman(currentLapDelta)}</div>
                      <div style={{ fontFamily: FONT_DIGITS, fontSize: 13, color: COLORS.text, minWidth: 60, textAlign: 'right' }}>{fmtCompact(ms)}</div>
                      <div style={{ width: 16 }} />
                    </div>
                  );
                })()}
                {[...t.laps].reverse().map((l, ri) => {
                  ...
                  return (
                    <div key={l.id} onClick={() => ctrl.requestEditLap(t, l.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: ri === t.laps.length - 1 ? 'none' : `1px solid ${COLORS.border}`, cursor: 'pointer' }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); api.deleteLap(t.id, l.id); }} style={{ background: 'transparent', border: 'none', color: COLORS.textMuted, cursor: 'pointer', padding: 4, fontSize: 12 }}>✕</button>
                    </div>
                  );
                })}
              </div>
