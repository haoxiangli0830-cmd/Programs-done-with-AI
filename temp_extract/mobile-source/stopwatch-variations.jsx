...
function EditLapSheet({ open, initialName, initialDelta, accent, onConfirm, onCancel }) {
  const [name, setName] = React.useState('');
  const [h, setH] = React.useState(0);
  const [m, setM] = React.useState(0);
  const [s, setS] = React.useState(0);
  ...
  React.useEffect(() => {
    if (open) {
      setName(initialName || '');
      setH(Math.floor(initialDelta / 3600000));
      setM(Math.floor(initialDelta / 60000) % 60);
      setS(Math.floor(initialDelta / 1000) % 60);
    }
  }, [open, initialName, initialDelta]);
  ...
  return (
    <div onClick={onCancel}
      ...
      <div onClick={(e) => e.stopPropagation()}
        style={{ width: '100%', background: COLORS.surface, borderTop: `1px solid ${COLORS.border}`, borderRadius: '20px 20px 0 0', padding: '20px 20px 28px', fontFamily: FONT_UI, color: COLORS.text }}>
        <div style={{ width: 36, height: 4, background: COLORS.border, borderRadius: 2, margin: '-8px auto 14px' }} />
        <div style={{ fontSize: 17, fontWeight: 600, marginBottom: 4 }}>Edit lap</div>
        <div style={{ fontSize: 13, color: COLORS.textDim, marginBottom: 16 }}>Edit the lap name and its duration.</div>
        
        <input value={name} onChange={(e) => setName(e.target.value)}
          placeholder="Lap name"
          style={{ width: '100%', boxSizing: 'border-box', padding: '12px 14px', marginBottom: 16, background: COLORS.surface2, border: `1px solid ${COLORS.border}`, borderRadius: 10, color: COLORS.text, fontFamily: FONT_UI, fontSize: 16, outline: 'none' }} />
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
          <button onClick={onCancel} style={{ flex: 1, padding: '12px', background: 'transparent', border: `1px solid ${COLORS.border}`, borderRadius: 10, color: COLORS.textDim, fontFamily: FONT_UI, fontSize: 15, fontWeight: 500, cursor: 'pointer' }}>Cancel</button>
          <button onClick={() => onConfirm(name || initialName, h * 3600000 + m * 60000 + s * 1000)}
            style={{ flex: 2, padding: '12px', background: accent, border: 'none', borderRadius: 10, color: '#000', fontFamily: FONT_UI, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>Save lap</button>
        </div>
      </div>
    </div>
  );
...
function useController(api) {
  ...
  const [editingLap, setEditingLap] = React.useState(null);

  const requestEditLap = (timer, lapId) => {
    const lapIdx = timer.laps.findIndex(l => l.id === lapId);
    if (lapIdx === -1) return;
    const l = timer.laps[lapIdx];
    const prev = lapIdx === 0 ? 0 : timer.laps[lapIdx - 1].atMs;
    const delta = l.atMs - prev;
    setEditingLap({ timerId: timer.id, lapId, name: l.name, delta, accent: timer.color });
  };
  const confirmEditLap = (name, deltaMs) => {
    if (editingLap) {
      api.editLap(editingLap.timerId, editingLap.lapId, name, deltaMs);
      setEditingLap(null);
    }
  };
  ...
  return {
    ...
    editingLap, requestEditLap, confirmEditLap, cancelEditLap: () => setEditingLap(null),
  };
}
...
function ControllerOverlays({ ctrl }) {
  return (
    <>
      ...
      <EditLapSheet open={!!ctrl.editingLap} accent={ctrl.editingLap?.accent} initialName={ctrl.editingLap?.name} initialDelta={ctrl.editingLap?.delta || 0} onConfirm={ctrl.confirmEditLap} onCancel={ctrl.cancelEditLap} />
    </>
  );
}
...
Object.assign(window, { VariationList, VariationCards, COLORS, FONT_UI, FONT_DIGITS, ControllerOverlays, LapModal, SetTimeSheet, EditLapSheet, ConfirmDialog, Digits, useController });
